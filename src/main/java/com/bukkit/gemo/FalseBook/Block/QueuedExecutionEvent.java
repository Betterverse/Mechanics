...
public class QueuedExecutionEvent
{
  ...
  public void Execute() {
    ...
    if ((ConfigHandler.isRespectLWCProtections(this.block.getWorld().getName())) && 
      (LWCProtection.getProtection(this.block).equalsIgnoreCase("private"))) {
      return;
    }
    ...
    } else if ((signBlock.getLine(1).equalsIgnoreCase("[Gate]")) || (signBlock.getLine(1).equalsIgnoreCase("[DGate]"))) {
      Block thisBlock = SignUtils.getSignAnchor(signBlock).getBlock();
      try
      {
        int TypeID = ConfigHandler.getAllowedGateBlocks(this.block.getWorld().getName()).getValue(0);
        if (signBlock.getLine(2).length() > 0) {
          TypeID = Integer.valueOf(signBlock.getLine(2)).intValue();
        }
        ((MechanicGate)this.plugin.getMechanicHandler().getMechanic("GATE")).toggle(thisBlock, signBlock.getLine(1).equalsIgnoreCase("[DGate]"), true, BlockUtils.isLow(this.redstoneLoc), TypeID);
      } catch (Exception e) {
        e.printStackTrace();
        return;
      }
