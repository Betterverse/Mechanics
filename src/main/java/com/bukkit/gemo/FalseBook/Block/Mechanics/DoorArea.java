...
public class DoorArea
{
  ...
  public boolean isBlockInArea(Block block)
  {
    ...
    if (BlockUtils.LocationEquals(this.sign2.getBlock().getLocation(), block.getLocation())) {
      return true;
    }

    if (BlockUtils.LocationEquals(this.sign1.getBlock().getRelative(BlockFace.DOWN).getLocation(), block.getLocation())) {
      return true;
    }
    if (BlockUtils.LocationEquals(this.sign2.getBlock().getRelative(BlockFace.DOWN).getLocation(), block.getLocation())) {
      return true;
    }

    if ((block.getX() < this.vector1.getBlockX()) || (block.getX() > this.vector2.getBlockX())) {
      return false;
    }

    if ((block.getZ() < this.vector1.getBlockZ()) || (block.getZ() > this.vector2.getBlockZ())) {
      return false;
    }

    return (block.getY() >= this.vector1.getBlockY()) && (block.getY() <= this.vector2.getBlockY());
  }
}