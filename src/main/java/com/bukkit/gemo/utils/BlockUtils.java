...
public class BlockUtils
{
  ...
  public static short getDataPart(String input) {
    try {
      return Short.valueOf(input.split(":")[1]).shortValue(); } catch (Exception e) {
    }
    return 0;
  }
  ...
  public static int getIDFromMatList(String itemName) {
    ...
    if (newName.equalsIgnoreCase(itemName)) {
      return ((Integer)matList.get(itemName.toLowerCase())).intValue();
    }
    return ((Integer)matList.get(newName.toLowerCase())).intValue();
  }
  ...
  public static boolean isValidItemID(String name, short Data) {
    ...
    return isValidItemID(((Integer)matList.get(name.toLowerCase())).intValue(), Data);
  }
